...
class GetSuhanPropertyInputDto(BaseModel):
    ...
    land_area_min: float | None = Field(default=None, description="최소 대지면적(㎡). 평 입력은 ㎡로 환산.")
    land_area_max: float | None = Field(default=None, description="최대 대지면적(㎡). 평 입력은 ㎡로 환산.")
    total_area_min: float | None = Field(default=None, description="최소 연면적(㎡). 평 입력은 ㎡로 환산.")
    total_area_max: float | None = Field(default=None, description="최대 연면적(㎡). 평 입력은 ㎡로 환산.")
    price_min: float | None = Field(default=None, description="매물 가격 최소(원). 만/억 입력은 원으로 변환.")
    price_max: float | None = Field(default=None, description="매물 가격 최대(원). 만/억 입력은 원으로 변환.")
