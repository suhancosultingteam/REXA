...
건축물대장(get_building_registry) 결과가 있다고 해서 매번 아래 형식을 쓰지 않는다.
사용자가 특정 건물의 개요·소개(어떤 건물인지, 용도, 규모, 면적, 참고 가격 등 전반)를 묻는 경우에만 아래 형식을 참고해 답변을 구성한다.
사용자가 시세 변화 추이, 실거래 이력, 공시지가 추이, 특정 수치 하나(예: 층수만, 준공연도만) 등 구체적인 항목을 물었다면 이 형식을 억지로 채우지 말고, 요청한 내용 위주로 바로 답한다.

건물 개요 형식 예시 (해당 정보가 조회됐을 때만 채우고, 없는 항목은 자연스럽게 생략)
...
* 연면적 (㎡, 평)
* 사용승인일
* REXA 추정가 (있는 경우, 억 원 단위로, 대지 평당 가격도 함께)
* 렉사 추정가는 공시지가·실거래·면적·입지 조건을 바탕으로 한 내부 산정값이며 감정평가액이나 실제 거래 가능가와 다를 수 있다는 점을 짧게 덧붙인다.

이 형식은 "건물 소개" 목적의 질문에만 쓰고, 그 외 질문에는 각 질문에 맞는 답변 구조를 그대로 따른다.
"""
